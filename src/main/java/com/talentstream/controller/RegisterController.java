...
public class RegisterController {
		...
		try {
			if (regsiterService.isGoogleSignIn(loginDTO)) {
				...
				return ResponseHandler.generateResponse("Login successfully" + userDetails.getAuthorities(),
						HttpStatus.OK, new AuthenticationResponse(jwt, refreshToken), applicant.getEmail(),
						applicant.getName(), applicant.getId(), applicant.getMobilenumber(), applicant.getUtmSource());
			} else {
	...
	public ResponseEntity<String> authenticateUser(@RequestBody PasswordRequest passwordRequest,
		...
		ResponseEntity<String> result = regsiterService.authenticateUser(id, decryptedOldPassword,
	...
	public ResponseEntity<Object> refreshToken(@RequestBody RefreshTokenDTO refreshTokenDTO) {
	    ...
	    try {
	        ...
	        if (isValid) {
	            ...
	            String newJwt = jwtTokenUtil.generateToken(email);
	            ...
	            logger.info("Returning new JWT token");
