...
public class ApplicantProfileService {
	...
	public String createOrUpdateApplicantProfile(long applicantId, ApplicantProfileDTO applicantProfileDTO)
		...
		if (applicant == null)
			...
			if (existingProfile == null) {
				...
				// Handle skills using replaceApplicantSkills to avoid duplicates
				if (applicantProfileDTO.getSkillsRequired() != null
						&& !applicantProfileDTO.getSkillsRequired().isEmpty()) {
					List<String> skillNames = applicantProfileDTO.getSkillsRequired().stream()
							.map(ApplicantSkills::getSkillName).collect(Collectors.toList());

					// Save profile first without skills
					applicantProfile.setSkillsRequired(new HashSet<>());
					applicantProfile = applicantProfileRepository.save(applicantProfile);

					// Then use replaceApplicantSkills to handle skill creation/association
					applicantKeySkillsService.replaceApplicantSkills(applicantId, skillNames);
				} else {
	...
	private ApplicantProfile convertDTOToEntity(ApplicantProfileDTO applicantProfileDTO) {
		...
		// Skills will be handled separately via replaceApplicantSkills
		applicantProfile.setSkillsRequired(new HashSet<>());
		...
		if (existingProfile == null) {
			...
			existingProfile.setSpecialization(updatedProfileDTO.getSpecialization());
			 ...
			 if (updatedProfileDTO.getSkillsRequired() != null) {
			        List<String> skillNames = updatedProfileDTO.getSkillsRequired().stream()
...
//	        	    applicantSkillBadgeRepository.deleteByApplicantIdAndSkillBadgeId(applicantId, skillBadge.getId());
			...
			// Handle skills using replaceApplicantSkills to avoid duplicates
			if (updatedProfileDTO.getSkillsRequired() != null && !updatedProfileDTO.getSkillsRequired().isEmpty()) {
				List<String> skillNames = updatedProfileDTO.getSkillsRequired().stream()
						.map(ApplicantProfileUpdateDTO.SkillDTO::getSkillName).collect(Collectors.toList());
				applicantKeySkillsService.replaceApplicantSkills(applicantId, skillNames);
			} 
			applicantProfileRepository.save(existingProfile);
		}
	...
	private void createEducationFromProfile(Applicant applicant, ApplicantProfile applicantProfile) {
		...
		try {
			...
			throw new CustomException("Failed to save education details: " + e.getMessage(),
					HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
